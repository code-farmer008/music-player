<template>
  <div class="player" v-show="playlist.length>0">
        ...
        <div class="bottom">
          ...
          <div class="operators">
            ...
            <div class="icon i-right">
              <i @click="toggleFavorite(currentSong)" class="icon" :class="getFavoriteIcon"></i>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class='toast' v-show='!canPlaySong'>
      <p>对不起，此歌曲暂不支持</p>
    </div>
  </div>
</template>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      ...
      updateTime (e) { // 播放时让audio的currentTime属性跟组件实例的currentTime同步
    ...
    watch: {
      async currentSong (newSong,oldSong) {
        this.songVkey = await getplaysongvkey(newSong.mid);
        ...
        if (!newSong.id || newSong.id === oldSong.id ) {
          ...
          let pureurl = `http://isure.stream.qqmusic.qq.com/${this.songVkey}`;
          this.$refs.audio.src = pureurl;
      ...
      fullScreen (newVal) {
        if (newVal) {
          setTimeout(() => {
            ...
            this.$refs.progressBar._offset(this.percent)
          }, 20)
        }else{
          this.canPlaySong = true
        }
